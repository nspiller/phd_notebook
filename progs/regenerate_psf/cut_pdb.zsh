...
grep SO rest.pdb >>sol.pdb
sed -i "/SO/d" rest.pdb
#grep -v SO rest3.pdb >rest4.pdb
linessol=`grep ATOM sol.pdb | wc -l | awk '{print $1}'`
echo "Initial large solvent segment contains $linessol atoms"
#Converting SO lines to TIP3 format
sed -i s'/OW  SO /OH2 TIP3/g' sol.pdb
sed -i s'/HW1 SO /H1  TIP3/g' sol.pdb
sed -i s'/HW2 SO /H2  TIP3/g' sol.pdb
...
# Converting deprotonated CYS back to CYS (not called CSD in QMMM)
# Will require patching by psfgen instead
# First grabbing list of deporotonated CYS residues
for i in `seq 1 $proteinfrag`
do
csdres=`grep -A1 'SG CYS' protein-$i.pdb | grep 'C CYS' | awk '{print $5}'`
...
then
  ...
  count=`echo $line | awk '{print $5}'`
        ...
        then
          #echo "Now cutting from $begincut to $linecount"
          sed -n "$begincut,$linecount p " extraspecies.pdb > extraspecies-$extraspeciesfrag.pdb
        fi
  prevcount=$count
  linecount=$((linecount + 1))
